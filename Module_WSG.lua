...
WSGFrame:SetScript("OnEvent", function(self, event, msg)
    local lowerMsg = string.lower(msg)
    
    if ns.DebugMode then
        print("|cff00ffff[WSG Module Raw]|r: " .. msg)
    end
...
local function OnEnable()
    WSGFrame:RegisterEvent("CHAT_MSG_BG_SYSTEM_ALLIANCE")
    WSGFrame:RegisterEvent("CHAT_MSG_BG_SYSTEM_HORDE")
    WSGFrame:RegisterEvent("CHAT_MSG_BG_SYSTEM_NEUTRAL")
    WSGFrame:RegisterEvent("CHAT_MSG_SYSTEM") -- Safety net for custom servers
...
local function OnDisable()
    WSGFrame:UnregisterAllEvents()
    wipe(ns.FlagCarriers)
end

if ns.RegisterModule then
    ns.RegisterModule("Warsong Gulch", OnEnable, OnDisable)
end